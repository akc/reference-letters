...
$\sigma$ is a permutation and $T$ is a sorting operator, then what is
the set $T^{-1}(\sigma)=\{\pi : T(\pi) = \sigma\}$? Of particular
interest is the set $T^{-1}(\Id_n)$ of permutations sortable by $T$.



If
$\Class$ is a \emph{pattern class}---i.e.\ a downwards closed set of
...
(stack-sort) or $\Q$ (queue-sort), and $\Class$ is of the form
$\Av(P)$, the set of permutations avoiding the patterns in
$P$. With this terminology, the permutations sortable by $T$ is
$T^{-1}(\Av(12))$.
...
They describe the sorting tree for $\B$. Explicitly describe $\unB(\sigma)$.
